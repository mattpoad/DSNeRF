...
def sample_sigma(rays_o, rays_d, viewdirs, network, z_vals, network_query, segmentation=False):
    ...
    rgb_map, disp_map, acc_map, weights, depth_map, seg_map = raw2outputs(raw, z_vals, rays_d, segmentation=segmentation)

    mask=[]
    if segmentation:
        mask = torch.sigmoid(raw[...,-3:])

    return rgb, mask, sigma, depth_map
